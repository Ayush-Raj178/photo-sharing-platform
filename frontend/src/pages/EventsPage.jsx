...
export function EventsPage({ role }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    setError(null)
    try { setEvents((await api.get('/events')).data.items) }
  ...
  return (
    <AppShell role={role}>
      ...
      {error ? <div style={{ marginTop: 14 }}><ErrorMessage error={error} onRetry={load} /></div> : null}
      {loading ? <LoadingState label="Loading events…" /> : events.length === 0 ? <EmptyState icon={CalendarDays} title={isAdmin ? 'Create your first event' : 'No assigned events yet'} description={isAdmin ? 'Events keep team uploads and customer galleries together.' : 'An Admin needs to assign you to an event before it appears here.'} action={isAdmin ? <button className="button button-primary" onClick={() => setCreating(true)}><Plus size={18} />Create event</button> : null} /> : <div className="event-list">{events.map((item) => <Link key={item.id} className="event-row" to={`/${isAdmin ? 'admin' : 'team'}/events/${item.id}`}><div><h2>{item.name}</h2><p>{item.description || 'No description'} · Created {formatDate(item.createdAt)}</p></div><ArrowRight aria-hidden="true" /></Link>)}</div>}
