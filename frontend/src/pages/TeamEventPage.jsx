...
export function validateFiles(files) {
  ...
  files.forEach((file) => {
    ...
    else if (file.size > MAX_FILE_SIZE) rejected.push(`${file.name}: larger than 4 MB.`)
...
export function TeamEventPage() {
  ...
  async function upload() {
    ...
    const MAX_BATCH_RAW_BYTES = 4 * 1024 * 1024
    const batches = []
    let currentBatch = []
    let currentSize = 0

    for (const item of pendingItems) {
      if (currentBatch.length > 0 && currentSize + item.file.size > MAX_BATCH_RAW_BYTES) {
    ...
    let totalFailed = 0
    let hasUncertainError = false

    for (const batchItems of batches) {
      const form = new FormData()
      batchItems.forEach((item) => form.append('files', item.file))
      try {
        const result = await api.post(`/events/${eventId}/photos`, form, {
          ...
          onUploadProgress: (progressEvent) => {
            ...
            setQueue((items) => items.map((item) => batchItems.some((candidate) => candidate.id === item.id) ? { ...item, progress } : item))
          },
        })
        ...
        totalFailed += result.data.results.length - succeeded
      } catch (nextError) {
        const parsed = getApiError(nextError, 'Upload could not be completed.')
        if (parsed.uncertain) hasUncertainError = true
        setQueue((items) => items.map((item) => batchItems.some((candidate) => candidate.id === item.id) ? { ...item, status: 'Failed', progress: 0, error: parsed.uncertain ? 'Outcome uncertain—review your photos before retrying.' : parsed.message } : item))
        totalFailed += batchItems.length
      }
    }

    if (hasUncertainError) {
      setError({ message: 'The server response was lost for some uploads. Your upload outcome is uncertain; the photo list was refreshed so you can review before retrying.' })
    }

    setSuccess(totalFailed ? `${totalSucceeded} uploaded; ${totalFailed} failed. Review the results below.` : `${totalSucceeded} photo${totalSucceeded === 1 ? '' : 's'} uploaded successfully.`)
